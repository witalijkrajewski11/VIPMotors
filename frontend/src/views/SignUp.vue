      ...
      <div class="section-main">
        <div class="section-login-1">
          <div class="section-login-1-main">
            ...
            <p class="section-login-1-text" style="color: black; margin-bottom: 7rem">Driving Excellence, Embracing Luxury</p>
            <div class="section-login-1-img" style=" margin-bottom: 11rem">
              <img
        ...
        <div class="section-login-2">
          <div class="section-login-2-main">
            ...
            <form class="section-login-2-form" @submit.prevent="registerUser">
              <div class="login-form-1">
                <label for="input-email">Email</label>
                <input type="email" id="input-email" placeholder="john@example.com" required v-model="email">
              </div>
              <div class="login-form-2">
                <label for="input-password">Password</label>
                <input type="password" id="input-password" placeholder="At least 8 characters" minlength="8" required
                       v-model="password">
              </div>
              <div class="login-form-3">
                <label for="input-password">Confirm Password</label>
                <input type="password" id="input-password" placeholder="At least 8 characters" minlength="8"
                       v-model="confirmPassword">
              </div>
              <div class="login-form-4">
                <input type="checkbox" id="input-checkbox" v-model="acceptTermsAndConditions">
                <p>By creating an account, you agree to the <router-link to="/terms">Terms & Conditions.</router-link></p>
              </div>
              <div class="login-form-submit-btn">
                <button :disabled="!acceptTermsAndConditions" type="submit">Create an Account</button>
              </div>
...
export default {
  ...
  mounted() {
    document.title = 'Sign Up | VIPMotors'
  },
  methods: {
    registerUser() {
      this.errors = []
      ...
      if (this.email[0] === ' ') {
        this.errors.push('Username can\'t starts with white space.')
      }
      if (this.password === '') {
        this.errors.push('Password can\'t be blank.')
      }
      if (this.password[0] === ' ') {
        this.errors.push('Password can\'t starts with white space.')
      }
      ...
      if (!this.errors.length) {
        const data = {
          ...
          'is_accepted_terms_and_conditions': this.acceptTermsAndConditions,
        }
        axios
            .post('api/users/', data)
            .then(res => {
              console.log(res.status)
              this.toast.success('Account created, please confirm your email!')
              if (res.status === 201) {
                this.onSuccessRegistration(res.data.email, res.data.password)
              }
            })
            .catch(e => {
              if (e.response) {
        ...
        for (let index = 0; index < this.errors.length; index += 1) {
          console.log(this.errors[index])
          this.toast.error(this.errors[index])
        }
      }
    },
    onSuccessRegistration(email, password) {
      axios
          .post('api/token/', {'username': email, 'password': password})
          .then(res => {
            console.log(res)
...
//background: url(https://rvs-gradie-signup-page.vercel.app/Assets/Background.png); background-repeat: no-repeat; background-size: cover; background-position: center; display: flex;
  align-items: center;
...
main section.section-login .section-main .section-login-2 .section-login-2-main .section-login-2-form .login-form-submit-btn button:hover {
